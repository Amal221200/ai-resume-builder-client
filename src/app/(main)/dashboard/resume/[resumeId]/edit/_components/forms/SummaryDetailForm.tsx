...
const SummaryDetailForm = ({ enableNav }: { enableNav: (val: boolean) => void }) => {
    ...
    const onGenerateAISummary = useCallback(async () => {
        setGenerating(true)
        const response = await generateAISummary(PROMPT.replace('{jobTitle}', resumeInfo.jobTitle ?? ""))
            ...
            <form onSubmit={onSubmit}>
                <div className='mt-5'>
                    <div className='space-y-2'>
                        ...
                        <Textarea rows={6} placeholder='Summary' id={summary} required name='summary' value={resumeInfo?.summary ?? ''} onChange={handleInput} />
                    </div>
                </div>
